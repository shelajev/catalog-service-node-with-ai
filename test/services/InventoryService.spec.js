...
describe("InventoryService", () => {
  const { getInventoryForProduct } = require('../../src/services/InventoryService');
  ...
  it("should return an error when the response is not 200", async () => {
    fetch.mockResolvedValueOnce({ status: 500, json: jest.fn().mockResolvedValueOnce({ message: "Some error" }) });
  ...
  it("should return the quantity when the request is successful", async () => {
    fetch.mockResolvedValueOnce({ status: 200, json: jest.fn().mockResolvedValueOnce({ quantity: 10 }) });
